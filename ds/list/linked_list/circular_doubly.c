...
void insert(int data, int location) {
  ...
  if (is_empty()) {
    ...
    if (location == 1) {
      ...
      new_node->previous = head_node->previous;
      (head_node->previous)->next = new_node;
...
void delete(int location) {
  ...
  if (location == 1 && size > 1) {
    ...
    head_node->previous = traverse_node->previous;
    (traverse_node->previous)->next = head_node;
  } else {
    while (location > 1) {
    ...
    (traverse_node->next)->previous = previous_node;
  }
  free(traverse_node);
  size--;
}
...
void reverse() {
  ...
  traverse_node = head_node;
  previous_node = head_node->previous;
  do {
    next_node = traverse_node->next;
    traverse_node->next = previous_node;
    traverse_node->previous = next_node;
    previous_node = traverse_node;
...
int main() {
  ...
  printf("\033[35;01mCircular Doubly Linked List\033[00m\n1 - Insert\n2 - Delete\n3 - Search\n4 - Display\n5 - Reverse\n6 - Exit\n");
  while (1) {
    printf("\nEnter option: ");
    scanf("%d", &option);
    ...
    switch (option) {
      ...
      case 6:
        ...
        return 0;
      default:
        printf("\033[31;01mError:\033[00m Invalid operation.\n");
    }
  }
}
