...
void insert(int data, int location) {
  ...
  if (is_empty()) {
    ...
    if (location == 1) {
      ...
      if (traverse_node->next != NULL) (new_node->next)->previous = new_node;
...
void delete(int location) {
  ...
  if (location == 1) {
    ...
    if (traverse_node->next != NULL) head_node->previous = NULL;
  } else {
    while (location > 1) {
    ...
    if (traverse_node->next != NULL) (traverse_node->next)->previous = previous_node;
  }
  free(traverse_node);
  size--;
}
...
int main() {
  ...
  printf("\033[35;01mDoubly Linked List\033[00m\n1 - Insert\n2 - Delete\n3 - Search\n4 - Display\n5 - Reverse\n6 - Exit\n");
  while (1) {
    printf("\nEnter option: ");
    scanf("%d", &option);
